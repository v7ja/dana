...
b3kkk = Client("Channel - B3KKK",api_id=29335417,api_hash="01b9e072189b8955d964733b7b4b921e",bot_token="6872790879:AAF-j8qRC6iOnTXGck9roeRhkePlPczi6hY")
...
async def START(c:Client,m:Message):
	...
	ارسل رسالتك وسيتم الرد عليك قريبا
		""",reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton ("Dev",user_id=6973422633)]]),quote= True)
	else:
		db.execute("INSERT INTO USERS(user_id) VALUES(?)", (m.from_user.id,))
		con.commit()
		try:
			await b3kkk.send_message(6973422633,f"""
		<u>«**New User**»</u>
...
@b3kkk.on_message(filters.command("تفعيل التواصل","") & filters.user(6973422633) & filters.private)
...
@b3kkk.on_message(filters.command("تعطيل التواصل","") & filters.user(6973422633) & filters.private)
...
@b3kkk.on_message(filters.command("الاحصائيات","") & filters.user(6973422633) & filters.private)
...
@b3kkk.on_message(filters.command("اذاعه للكل","") & filters.user(6973422633) & filters.private)
...
@b3kkk.on_message(filters.command("حظر عضو","") & filters.user(6973422633) & filters.private)
...
@b3kkk.on_message(filters.command("الغاء حظر عضو","") & filters.user(6973422633) & filters.private)
...
@b3kkk.on_message(filters.private & ~filters.command("start") & ~filters.user(6973422633))
async def Private(c:Client,m:Message):
	...
	if CHECK_BAND(user_id=m.from_user.id):
		...
		await b3kkk.copy_message(chat_id=6973422633,
